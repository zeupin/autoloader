...
class Autoloader
{
    ...
    public static function dump()
    {
        ...
        foreach (self::$_queue as $item) {
            switch ($item['type']) {
                case 'classmap':
                    self::dumpClassmap($classmap, $item['mapfile'], $item['basedir']);
                    continue;

                case 'namespace':
                    self::dumpNamespace($classmap, $item['namespace'], $item['basedir']);
                    continue;

                case 'psr4':
                    self::dumpPsr4($classmap, $item['namespace'], $item['basedir']);
                    continue;

                case 'psr0':
                    self::dumpPsr0($classmap, $item['namespace'], $item['basedir']);
                    continue;
            }
        }
     ...
     */
    private static function dumpClassmap(array &$classmap, $namespace, $basedir)
    {
     ...
     * @param string $basedir
     */
    private static function dumpNamespace(array &$classmap, $namespace, $basedir)
    {
     ...
     * @param string $basedir
     */
    private static function dumpPsr0(array &$classmap, $namespace, $basedir)
    {
     ...
     * @param string $basedir
     */
    private static function dumpPsr4(array &$classmap, $namespace, $basedir)
    {
        if (!is_dir($basedir)) {
            return;
        }
        ...
        foreach ($files as $file) {
            ...
            if (is_dir($path)) {
                self::dumpPsr4($classmap, $namespace . '\\' . $file, $path);
                continue;
            }
